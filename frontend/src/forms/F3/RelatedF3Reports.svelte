<script lang="ts">
  ...
  async function fetchCandidateReports(): Promise<CandidateReport[]> {
    if (!reportCode || !coverageThroughDate || !filerId) return [];
  ...
  const reports = useQuery(fetchCandidateReports);
...
{:else if reports.data && reports.data.length > 1}
  <div class="related-reports">
    <h4>Candidates in Race</h4>
